...
- happy-app's recovery from a lost event is **HTTP re-fetch**: `fetchSessions` calls `tunnelFetch('/v1/sessions', ...)` (`packages/happy-app/sources/sync/sync.ts:1135-1195`). `fetchMessages` and `loadOlder` call `apiSocket.requestForSession(sessionId, '/v3/sessions/:id/messages?...')` — which **despite the method name** is also `tunnelFetch` under the hood (`packages/happy-app/sources/sync/apiSocket.ts:282-298`).
