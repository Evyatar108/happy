...
Agent-readable Ralph pipeline state is generated at `plans/overview-snapshot.json`; recent transitions append to `plans/overview-activity.jsonl`. Both are generated by `scripts/sync-ralph-state.mjs --watch`, which is auto-started by `pnpm overview`.
