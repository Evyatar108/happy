...
Before validating a release against Sprint E private-tunnel pairing, verify the Microsoft Dev Tunnels CLI is installed with `devtunnel --version`. Pairing uses private tunnels plus `X-Tunnel-Authorization: tunnel <connect-jwt>` (Microsoft's gateway auth, stripped before backend) as the sole identity gate for remote callers. The earlier custom `X-Codexu-Authorization` tunnel-claim header was retired by the remove-tunnel-claim-layer effort — happy-server, happy-app, happy-cli, and happy-agent no longer mint, send, or verify it. Do not add anonymous tunnel access as a release workaround.
