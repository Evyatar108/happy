...
Before validating a release against Sprint E private-tunnel pairing, verify the Microsoft Dev Tunnels CLI is installed with `devtunnel --version`. Pairing uses private tunnels plus `X-Tunnel-Authorization: tunnel <connect-jwt>` (Microsoft's gateway auth, stripped before backend) and `X-Codexu-Authorization: tunnel <happy-claim>` (custom header consumed by happy-server). The Sprint A spec had the header names backwards relative to the gateway's actual auth scheme — see `packages/happy-app/scripts/sprint-a-gap.md` "R-D18 path (b) implementation log". Do not add anonymous tunnel access as a release workaround.
