...
11. **`connectRoutes.ts` redirect still points to `https://app.happy.engineering`.** Must be updated to `happy://` deep link scheme before Happy GitHub OAuth proxy works from mobile. `app.config.js` has `scheme: 'happy'` but `_layout.tsx` has no handler for this OAuth return.
