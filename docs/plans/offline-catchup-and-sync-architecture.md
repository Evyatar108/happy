...
| Claim | Verdict | Evidence |
|---|---|---|
| Codex uses a JSONL tailer like Claude | ❌ FALSE | Codex uses JSON-RPC over stdio with `codex app-server` child (`codexAppServerClient.ts:393, :468, :954`). No tailer. Grep for `scanner|tail|watcher|jsonl` under `src/codex/` returns no matches. |
| "Happy offline during Codex activity" loses messages | N/A by construction | If happy-cli isn't running, no `codex app-server` child is running either, so there are no events being emitted. The Claude-style "data exists on disk but happy-cli didn't read it" gap doesn't apply. |
| happy-cli crash mid-turn loses in-flight events | ❌ TRUE (real Codex gap) | Events emitted by the app-server child between the last successful socket forward and happy-cli's exit are lost. The `pendingOutbox` is in-memory only; SIGTERM/SIGKILL kills the child too (`codexAppServerClient.ts:504-517`). |
...
|---|---|---|
| Happy offline + agent CLI continues | ❌ Lost (orphan JSONL not enumerated) | ✅ N/A by construction (no app-server without happy-cli) |
| happy-cli crash mid-turn → in-flight events | ⚠️ JSONL still on disk; not replayed on restart | ❌ Lost (in-memory outbox dies) |
