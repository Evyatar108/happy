...
## What's in `feature/tablet-sidebar-toggle` (in commit order, current tip `c98bb557`)
...
7. **`feat(markdown): strip Claude Code command-metadata tags (self-discovering)`** + **`feat(markdown): render Claude Code metadata tags instead of showing raw`** — preprocessor in `MarkdownView` that transforms `<command-name>` / `<local-command-stdout>` / etc. into readable markdown rather than showing the raw tags. Taxonomy + rationale in the "Claude Code metadata tags" section below; discovery workflow captured in `.agents/skills/happy-discover-metadata-tags/SKILL.md`.
8. **`feat(chat): expand chatFontScale beyond markdown to cover the whole chat`** — the Appearance → Chat text size setting now also scales code blocks (via new `fontSize`/`lineHeight` props on `SimpleSyntaxHighlighter`), agent-event notices, tool section titles, Bash output (via `CommandView`), and the composer (`MultiTextInput`). Shared hook extracted to `sources/hooks/useChatFontScale.ts`. Residual per-tool-view typography (Edit, Write, Grep, Task, Todo, etc.) still unscaled — tracked in `docs/fork-roadmap.md`.
...
- **Font scale scope — partially done.** Covers markdown (body / headers / lists / code blocks + language label), agent-event notices, tool section titles, Bash output (via `CommandView`), and the composer. **Still not scaling**: the per-tool views beyond Bash — EditView, WriteView, GrepView, TaskView, TodoView, GeminiExecuteView, etc., each with their own typography. Mechanical to add. See `docs/fork-roadmap.md` item 1 for the full list.
