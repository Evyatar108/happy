...
On the server:
- `socket.ts` authenticates the handshake via per-listener auth mode (`StartSocketOptions.auth`). On the tunnel listener, `verifyTunnelClaim(authHeader, tofuConfig)` validates `X-Tunnel-Authorization: tunnel <envelope>` — a base64url Ed25519-signed envelope `{ p, s }` where the signature is verified against `tofuConfig.tofuPublicKeys.ed25519PublicKey`, then `payload.sub === localUserId` and `iat` within 24 hours are enforced. The previous unsigned/Dev Tunnels JWT fallback has been removed; only signed Happy Ed25519 envelopes are accepted. On the loopback listener, the handshake validates `X-Loopback-Capability` against the on-disk capability file (mtime-cached, see `auth/loopbackCapability.ts`) and pins `socket.data.userId = tofuConfig.localUserId`. A cross-presented claim is rejected on the wrong listener. After validation it tags the socket with `socket.data.userId` (always the configured `localUserId` in the per-machine architecture), `clientType`, scope metadata, and the embedded server's TOFU public keys (`socket.data.tofuPublicKeys`), then enables the Redis streams adapter when `REDIS_URL` is set. On `connection`, if TOFU public keys are configured the server immediately emits a `tofu-pubkeys` event so the client can pin them.
