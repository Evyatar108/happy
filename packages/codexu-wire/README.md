...
`codexu-wire` centralizes definitions for:
...
In this repository, consumer workspaces import `codexu-wire` through package exports that point at `dist/*`.

That means on a clean checkout:
1. Build wire first: `yarn workspace codexu-wire build`
...
yarn workspace codexu-wire build
yarn workspace codexu-wire test
```
