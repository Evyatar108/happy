...
function loadConfig() {
  ...
  const homeDir = process.env.HAPPY_AGENT_HOME_DIR ?? node_path.join(node_os.homedir(), ".happy-agent");
  ...
  const credentialPath = node_path.join(homeDir, "credentials.json");
...
function loadCredentials(config) {
  if (!node_fs.existsSync(config.credentialPath)) {
...
`;
  node_fs.writeFileSync(tmpPath, data, { flag: "wx", mode: 384 });
  const fd = node_fs.openSync(tmpPath, "r");
  try {
    fsyncBestEffort(fd);
  } finally {
  ...
  if (process.platform !== "win32") {
    node_fs.chmodSync(tmpPath, 384);
  }
  for (let attempt = 1; ; attempt++) {
    try {
      node_fs.renameSync(tmpPath, config.credentialPath);
      break;
    } catch (error) {
      if (error.code !== "EBUSY" || attempt >= 3 || process.platform !== "win32") {
        throw error;
      }
      await promises.setTimeout(50 * attempt);
    }
  }
  try {
    const dirFd = node_fs.openSync(node_path.dirname(config.credentialPath), "r");
    try {
      fsyncBestEffort(dirFd);
    } finally {
      node_fs.closeSync(dirFd);
    }
  } catch {
  }
}
...
function mapTunnel(tunnel) {
  ...
  return happyWire.MachineTunnelSchema.parse({
  ...
  for (const discovered of status.discoveredMachines ?? []) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (error instanceof axios.AxiosError && error.response?.status) {
...
function readLegacyCredentials() {
  const legacyPath = node_path.join(process.env.HAPPY_HOME_DIR ?? node_path.join(node_os.homedir(), ".happy"), "agent.key");
  if (!node_fs.existsSync(legacyPath)) return {};
  const parsed = JSON.parse(node_fs.readFileSync(legacyPath, "utf-8"));
...
function isAxiosStatus(error, status) {
  return error instanceof axios.AxiosError && error.response?.status === status;
}
function sleep(ms) {
...
async function callMachineRpc(tunnelUrl, connectToken, method, params) {
  ...
  const socket = socket_ioClient.io(tunnelUrl, {
...
async function appendLedgerRecord(runId, sessionId, record) {
  const parsed = happyWire.LedgerRecordSchema.parse(record);
  if (parsed.runId !== runId || parsed.sessionId !== sessionId) {
    throw new Error("Ledger record identity does not match target ledger path");
  }
  const ledgerDir = node_path.join(process.env.HAPPY_PROJECT_PATH ?? process.cwd(), ".ralph", "state", runId);
  await promises$1.mkdir(ledgerDir, { recursive: true });
  await promises$1.appendFile(node_path.join(ledgerDir, `${sessionId}.jsonl`), `${JSON.stringify(parsed)}
`, "utf8");
}
...
async function readSessionLedger(projectPath, runId, sessionId) {
  try {
    const text = await promises$1.readFile(node_path.join(projectPath, ".ralph", "state", runId, `${sessionId}.jsonl`), "utf8");
    const records = [];
    for (const line of text.split(/\r?\n/).filter((l) => l.trim().length > 0)) {
      const result = happyWire.LedgerRecordSchema.safeParse(JSON.parse(line));
      if (result.success) {
...
program.command("auth").description("Manage authentication").addCommand(
  new commander.Command("login").description("Authenticate via GitHub device flow").action(async () => {
