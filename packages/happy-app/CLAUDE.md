...
- Active-session model / permission / effort controls live in `sources/components/AgentInput.tsx`'s `FloatingOverlay`. `sources/-session/SessionView.tsx` owns the picker callbacks: update local storage first, then emit live config through `sources/sync/ops.ts` `sessionEmitAgentConfiguration(...)`. `SessionContextDrawer.tsx` is confirmatory only for machine/path/model/permission chips plus resume/fork actions.
