...
- `sources/components/markdown/skillBody.ts` - detector for Claude Code's post-Skill-tool injection. After a `Skill` tool_use/tool_result, Claude Code posts a verbatim copy of the loaded `SKILL.md` as a `user`-role text message prefixed with `Base directory for this skill: <abs-path>\n\n# <Heading>`. The wrench-icon Skill ToolView already shows the call, so `UserTextBlock` in `MessageView.tsx` calls `isSkillBodyMessage(text)` and returns `null` to hide the entire bubble. Keep the regex strict (anchor + double-newline + `# `) so user messages mentioning the prefix in passing don't get suppressed.
