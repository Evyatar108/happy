...
describe('matchesTaskFilter — search haystack parity (F-011 regression)', () => {
    it('matches against workstream label (data.workstream lookup)', () => {
        ...
        expect(matchesTaskFilter(task, data, createEmptyFilters(), 'observability', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })

    it('matches against scope keyword', () => {
        const task = makeTask({ scope: 'bookkeeping' })
        expect(matchesTaskFilter(task, makeData(), createEmptyFilters(), 'bookkeeping', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })

    it('matches against cadence label (data.cadence lookup)', () => {
        const task = makeTask({ id: 't1' })
        const data = makeData({ cadence: { t1: 'weekly' } })
        expect(matchesTaskFilter(task, data, createEmptyFilters(), 'weekly', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })

    it('matches against size bucket (data.sizeBucket lookup)', () => {
        const task = makeTask({ id: 't1' })
        const data = makeData({ sizeBucket: { t1: 'epic' } })
        expect(matchesTaskFilter(task, data, createEmptyFilters(), 'epic', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })
    ...
    it('matches against warning html (rendered in row)', () => {
        ...
        expect(matchesTaskFilter(task, makeData(), createEmptyFilters(), 'upstream-foo', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })
    ...
    it('matches against spawned-from parent id', () => {
        ...
        expect(matchesTaskFilter(task, data, createEmptyFilters(), 'parent-investigation', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })
    ...
    it('matches against spawned-children ids (reverse lookup of data.spawnedFrom)', () => {
        ...
        expect(matchesTaskFilter(task, data, createEmptyFilters(), 'child-alpha', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
        expect(matchesTaskFilter(task, data, createEmptyFilters(), 'child-beta', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })

    it('matches against mergeCommit short SHA', () => {
        const task = makeTask({ mergeCommit: 'abc1234' })
        expect(matchesTaskFilter(task, makeData(), createEmptyFilters(), 'abc1234', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })

    it('matches against phase label', () => {
        const task = makeTask({ phase: 'impl-in-progress' })
        expect(matchesTaskFilter(task, makeData(), createEmptyFilters(), 'impl-in-progress', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })
    ...
    it('still matches against task.command.planPrompt', () => {
        ...
        expect(matchesTaskFilter(task, makeData(), createEmptyFilters(), 'prompt body', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })

    it('returns false when query matches nothing', () => {
        const task = makeTask({ id: 't1' })
        const data = makeData({ workstream: { t1: 'observability' } })
        expect(matchesTaskFilter(task, data, createEmptyFilters(), 'nonexistent', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(false)
    })

    it('is case-insensitive', () => {
        const task = makeTask({ scope: 'codexu' })
        expect(matchesTaskFilter(task, makeData(), createEmptyFilters(), 'CoDeXu', NO_TASK_ID_FILTER, NO_RALPH_STATE)).toBe(true)
    })
})
